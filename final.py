...
def run():
    ...
    while running:
        ...
        if len(indexes) > 0:
            ...
            for i in indexes.flatten():
                ...
                if roi_on == False:
                    ...
                    cv2.rectangle(img, (start_x+x, start_y+y), (x+start_x + w, y+start_y + h), color, 2)
                    text = label
                    # putText(프레임,텍스트,문자열 위치, 폰트,폰트 크기, 색상,굵기)
                    cv2.putText(img, text, (start_x+x, start_y+y + 10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
        ...
        # 파이큐티
            if ret:
                ...
                # Release Of The Mouse
                    ...
                    if count == 0 :
                        ROI = cv2.add(ROI, (200, 50, 50, 0))
                        #ROI = cv2.add(ROI, (200, 200, 50, 0))
                    elif count == 1 :
                        ROI = cv2.add(ROI, (0, 65, 200, 0))
                    else:
                        ROI  = cv2.add(ROI, (50, 50, 200, 0))
        ...
        elif redrectangle == False:
            if not (start_x == 0 and end_x == 0 and start_y == 0 and end_y == 0) :
                ROI = img[start_y: end_y, start_x: end_x]
                ROI = cv2.add(ROI, (50, 50, 200, 0))
                img[start_y: end_y, start_x: end_x] = ROI
